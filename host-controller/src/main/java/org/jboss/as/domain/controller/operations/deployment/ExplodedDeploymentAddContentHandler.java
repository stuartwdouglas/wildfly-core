...
public class ExplodedDeploymentAddContentHandler implements OperationStepHandler {
     ...
     */
    public ExplodedDeploymentAddContentHandler(final boolean backup, final HostFileRepository fileRepository, final ContentRepository contentRepository) {
    ...
    public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
        if (context.getProcessType() == ProcessType.SELF_CONTAINED) {
            throw DomainControllerLogger.ROOT_LOGGER.cannotAddContentToSelfContainedServer();
        }
        ...
        if (!isManaged(contentItem)) {
            throw DomainControllerLogger.ROOT_LOGGER.cannotAddContentToUnmanagedDeployment();
        } else if (isArchive(contentItem)) {
            throw DomainControllerLogger.ROOT_LOGGER.cannotAddContentToUnexplodedDeployment();
        }
        ...
        if (fileRepository == null && contentRepository != null) { //Master DC
            try {
                newHash = DeploymentUploadUtil.addContentToExplodedAndTransformOperation(context, operation, contentRepository);
